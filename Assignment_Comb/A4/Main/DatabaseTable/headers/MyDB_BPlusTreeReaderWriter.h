...
private:
	...
	// implementation of two get range iterator interfaces, sortedOrNot indicates whether an in-place sort is needed  
	MyDB_RecordIteratorAltPtr myGetRangeIteratorAlt (MyDB_AttValPtr low, MyDB_AttValPtr high, bool sortedOrNot);
	...
	// points to the record holding the (key, ptr) pair pointing to the new page.  Note that the new page
	// always holds the lower 1/2 of the records on the page; the upper 1/2 remains in the original page
	MyDB_RecordPtr split (MyDB_PageReaderWriter splitMe, MyDB_RecordPtr appendMe);
	...
	void printing (int whichpage, int depth);
