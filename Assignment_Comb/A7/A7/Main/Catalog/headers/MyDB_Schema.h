...
public:
	...
	// update attribute name by appending tableName + "_" ahead
	void updateAttName (string tableName);

	// retrieve the original attribute name
	void retrieveAttName();
...
private:
	...
	// original value for the schema attributes
	vector <pair <string, MyDB_AttTypePtr>> originAllAtts;
};
