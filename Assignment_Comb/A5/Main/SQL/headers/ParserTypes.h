...
private:
	...
	map <string, map <string, string>> allInfo;
...
public:
	...
	bool check (map<string, map<string, string>> allInfo) {
		this->allInfo = allInfo;
		for (auto pair : tablesToProcess)
			tableDef[pair.second] = pair.first;
		
		return (tableCheck() && attrCheck() && aggrCheck() && opCheck());
	}
	
	bool tableCheck () {
		for (auto it = tableDef.begin(); it != tableDef.end(); it++) {
			if (allInfo.find(it->second) == allInfo.end()) {
				cout << "------------- [ ERROR ] -------------" << endl;
				cout << "------- [ Table NOT Existed ] -------" << endl;
				cout << "[ FROM ] Table \"" + it->second + "\" NOT existed." << endl;
				return false;
			}
		}
		return true;
	...
	bool attrCheck () {
		for (auto expr : valuesToSelect) {
			if (!expr->exprAttrCheck(tableDef, allInfo, "SELECT")) return false;
		}
		for (auto expr : allDisjunctions) {
			if (!expr->exprAttrCheck(tableDef, allInfo, "WHERE")) return false;
		}
		for (auto expr : groupingClauses) {
			if (!expr->exprAttrCheck(tableDef, allInfo, "GROUP_BY")) return false;
		}
		return true;
	}
	...
	bool opCheck () {
		for (auto expr : valuesToSelect) {
			if (!expr->exprOpCheck(tableDef, allInfo, "SELECT")) return false;
		}
		for (auto expr : allDisjunctions) {
			if (!expr->exprOpCheck(tableDef, allInfo, "WHERE")) return false;
		}
		return true;
	}

	bool aggrCheck () {
		vector<pair<string, string>> groupByAttrs;
		for (auto expr : groupingClauses) {
			expr->addGroupByAttrs(groupByAttrs);
		}
		if (groupByAttrs.size() == 0) return true;

		for (auto expr : valuesToSelect) {
			if (!expr->exprAggrCheck(groupByAttrs)) return false;
		}
		return true;
	}
...
public:
	...
	void processSFWQuery (map<string, map<string, string>> allInfo) {
		if (myQuery.check(allInfo)) {
			cout << "---------- [ PASS ] ----------" << endl;
			cout << "Great! Query check passed." << endl;
		}
	}
