...
void MyDB_Schema :: updateAttName (string tableName) {

	// appending table name at the beginning of each attribute name
	vector<pair<string, MyDB_AttTypePtr>> newAtts;
	vector<pair<string, MyDB_AttTypePtr>> originAtts;
	for (auto a : allAtts) {
		string newAttName = tableName + "_" + a.first;
		newAtts.push_back(make_pair(newAttName, a.second));
		originAtts.push_back(make_pair(a.first, a.second));
	}
	
	allAtts = newAtts;
	originAllAtts = originAtts;
}

void MyDB_Schema :: retrieveAttName() {
	
	// retrieve the original attribute name
	vector<pair<string, MyDB_AttTypePtr>> originAtts;
	for (auto a : originAllAtts) {
		originAtts.push_back(make_pair(a.first, a.second));
	}
